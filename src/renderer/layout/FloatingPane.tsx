...
export function FloatingPane({
  ...
  onActivate, onGeometryChange, children
}: Props) {
  ...
  useEffect(() => {
    if (!resizingRef.current) setLiveGeo({ x, y, width, height })
  }, [x, y, width, height])
  ...
  const startResize = (dir: ResizeDir) => (e: React.MouseEvent) => {
    ...
    const compute = (ev: MouseEvent) => {
      ...
      if (dir === 'e') nw = Math.max(MIN_W, nw + dx)
      if (dir === 's') nh = Math.max(MIN_H, nh + dy)
      if (dir === 'w') {
