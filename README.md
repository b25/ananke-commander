...
- **Browser** — `WebContentsView`, isolated session. Navigation is a **scheme allowlist, not a host allowlist**: any `http(s)` URL with a hostname is allowed (the pane is a general-purpose browser, and non-URL input is routed to a web search), while `javascript:`, `file:`, `vbscript:`, `chrome:`, `chrome-extension:`, and `data:` are blocked, and only `about:blank` is allowed for `about:` (see `isNavigationAllowed` in `src/main/security/browserSecurity.ts`). Recent URL list is capped by **Settings → Browser recent URLs**.
